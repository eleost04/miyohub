...
import NetworkProxyEditor from './NetworkProxyEditor.vue'
...
async function save() {
  ...
  try {
    ...
    if (JSON.stringify(draft.value) === JSON.stringify(sent)) draft.value = saved
      ...
      reconcileCaptcha(draft.value.captcha.channels, sent.captcha.channels, saved.captcha.channels)
      const current = draft.value.network.proxy, before = sent.network.proxy, after = saved.network.proxy
      if (current.url === before.url && current.username === before.username) {
        current.has_password = after.has_password
        if (current.password === before.password) current.password = after.password
        if (current.clear_password === before.clear_password) current.clear_password = after.clear_password
      }
    }
  ...
  <form ref="form" class="settings-form" @submit.prevent="save">
    ...
    <fieldset :disabled="!admin" class="settings-fields">
      ...
      <article class="panel"><div class="panel-title"><div><p class="eyebrow">站点网络</p><h2>连接与重试</h2></div></div>
        ...
        <NetworkProxyEditor v-model="draft.network.proxy" :disabled="!admin" />
      </article>
      ...
      <article class="panel"><div class="panel-title"><div><p class="eyebrow">兑换设置</p><h2>兑换基础服务</h2></div><span class="pill soft">需单独授权</span></div><label class="preference-switch"><span><strong>允许商品兑换</strong><small>开启后仍需在「后台管理」为普通用户授予兑换权限；关闭不会删除历史计划。</small></span><span class="push-switch"><input v-model="draft.shop_exchange.enable" type="checkbox" aria-label="允许商品兑换" /><span aria-hidden="true"></span></span></label>
        <details class="preference-advanced"><summary>重试与保护参数<AppIcon name="chevron" :size="14" /></summary><p class="muted">明确返回兑换失败、未到兑换时间或繁忙时，在窗口内继续尝试。成功、售罄、余额不足、限购或需要验证时停止；结果不明的超时请求不会重发。</p><div class="form-grid"><label>重试窗口（秒，0 为仅请求一次）<input v-model.number="draft.shop_exchange.retry_seconds" type="number" min="0" max="120" step="0.1" required /></label><label>重试间隔（秒，实际不低于 0.2）<input v-model.number="draft.shop_exchange.retry_interval" type="number" min="0.05" max="30" step="0.05" required /></label></div><p class="muted">预约窗口从计划时间开始计算，准备或排队不会延长截止时间；即时兑换从本次执行开始计算。最多 120 秒、600 次请求，频繁提示至少等待 2 秒。同账号实际请求串行，截止后不再发起新请求，但会等待已发请求返回（最多 15 秒）。</p></details>
      </article>
    </fieldset>
