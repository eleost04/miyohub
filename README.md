...
界面使用小体积 SVG；ICO / iOS PNG 仅作兼容。页面、配置弹窗、二维码库和人机验证按需加载。静态 JS / CSS 预生成 Brotli、gzip，应用默认让哈希资源缓存一年（反代可以改写），入口页重新验证；私人 API 不缓存。商品图片通过受限官方 CDN 302 跳转，主站不下载图片；这不解决客户端无法访问 CDN 的情况。

参考占用（Linux/amd64）：主站为 2026-09-13 的 beta.2 样本，验证码服务为 2026-09-11 样本。镜像为未压缩大小；实际内存随负载变化：

| 服务 | 镜像 | 参考空闲内存 | 建议预留 |
| --- | ---: | ---: | --- |
| MiyoHub | 18.9 MiB | 约 14.4 MiB（启动并建立机器人连接后） | 1 核、256 MiB RAM |
...
首轮 amd64 / arm64 发布与发布后的私有可见性检查已在 [2026-09-13 的 CI](https://github.com/eleost04/miyohub/actions/runs/34727237905) 验证。双架构构建通过不代表已完成 arm64 真机验收。
