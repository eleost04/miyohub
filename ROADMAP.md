...
稳定基线：`0.0.1`。已发布测试版：`0.1.0-beta.1`。本轮目标：`0.1.0-beta.2`。实际发布状态见 [GitHub Releases](https://github.com/eleost04/miyohub/releases)，更新方式与提交规则见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
| PUSH-02 | fix | 签到通知过于冗长 | 修复中，目标 beta.2 | 仅保留账号 / 时间、签到结果、米游币增量与余额；失败及停止原因保留，成功步骤、常规跳过、重复汇总留在站内日志 |
| LOG-01 | fix | 汇总行无法查看完整过程、异常筛选误报 | 修复中，目标 beta.2 | 新日志按账号和执行编号关联；旧日志明确显示上下文；手机 / 桌面可查看明细，失败 0 不算异常，不改变用户隔离 |
