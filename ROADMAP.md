...
| ID | 类型 | 项目 | 状态 | 验收条件 |
| --- | --- | --- | --- | --- |
| AUTH-01 | fix | 短信登录 Geetest 3/4 兼容 | 修复中 | 两种挑战都能手动完成；发送与登录均可恢复；会话归属、过期、防重放与沙箱测试通过 |
| TASK-01 | fix | 米游币运行被无变化保存取消 | 修复中 | 复现并覆盖保存/调度变化；真正的停止和撤权仍生效；日志明确停止原因 |
| TASK-02 | feat | 增加可配置的状态查询重试 | 规划 | 展示实际重试次数；有限退避、可取消；不重放结果不确定的写请求 |
| NET-01 | feat | HTTP / SOCKS5 出站代理 | 规划 | 管理员可配置；凭据加密和脱敏；不放宽个人接口的 SSRF 防护；直连、代理和错误路径有测试 |
